  ...
  constructor() {
    super({
      ...
      void main() {
        ...
        // Tons azuis variados por partícula (azul escuro ↔ ciano claro)
        float colorHash = fract(sin(dot(vInitialPosition.xz, vec2(127.1, 311.7))) * 43758.5453);
        vec3 deepBlue = vec3(0.04, 0.28, 0.56);      // #0A4790
        vec3 brightBlue = vec3(0.10, 0.52, 0.85);     // #1A85D9
        vec3 lightCyan = vec3(0.37, 0.77, 0.93);      // #5EC4ED
        vec3 blueColor = mix(deepBlue, mix(brightBlue, lightCyan, smoothstep(0.5, 1.0, colorHash)), smoothstep(0.0, 0.5, colorHash));
        // Partículas mais brilhantes ficam mais claras
        blueColor = mix(blueColor, lightCyan, sparkleBrightness * 0.3);

        gl_FragColor = vec4(blueColor, mix(alpha, sparkleBrightness - 1.1, uTransition));
      }`,
      uniforms: {
